  ...
  return (
    ...
    <View className='flex-row'>
      <View className='flex-1 pr-2'>
        ...
        <TouchableOpacity onPress={() => {
          ...
          <Text className='px-4 py-3 font-bold text-center text-white'>장바구니에 추가하기</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          className={`rounded-lg mt-2 border${items.length > 0 ? "border=[#00ccbb]" : "border-gray-200"}`}
        >
          <Text className={`px-4 py-3 font-bold text-center ${items.length > 0 ? "text-[#00ccbb]" : "text-gray-200"}`}>장바구니에서 모든 아이템 삭제하기</Text>
        </TouchableOpacity>
      </View>
      ...
      <View>
        <Image
          ...
          className='object-cover w-20 h-20 p-4 bg-gray-300'
   ...
   {isPressed && (
    <View className='px-4 bg-white'>
      <View className='flex-row items-center bg-3 space-x-2'>
